...
destination_addr <- "Veltins-Arena, Gelsenkirchen"
...
subtitle = sprintf(
  "**The Revierderby** refers to the association football matches between 
  Borussia Dortmund and FC Schalke 04. 
  Outside Germany, it is often referred to as the **Ruhr derby**.<br><br>
  There have been 159 competitive matches between 1925 and 2022 (as of 17 September 2022).
  Dortmund have won 37 matches, Schalke 32, with 30 draws.<br><br>
  The stadiums are located only **%s** away.",
  distance$rows$elements[[1]]$distance$text
)

ragg::agg_png(here("plots", "02-lines-directions-bvb-s04.png"), res = 500, 
              units = "in", width = 7, height = 5)
ggmap(map_toner, darken = c(0.2, "grey10"), extent = "device") +
  geom_line(
  ...
  annotate(
    ...
    label = sprintf(
      "<img src='%s' width='35'>", 
      here("input", "team_icons", "FC Schalke 04.png")),
  ...
  annotate(
    ...
    fill = alpha("white", 0.9), box.size = 0.1, box.r = unit(1, "mm"),
    family = "Source Sans Pro", width = 0.5, size = 3
  ) +
  labs(
    title = "Road to Revierderby",
